...
<b><font color=red>WARNING! I have experienced problems with the docxtpl library on linux! I strongly advise using the Windows version of Burp to benefit from the automatic reporting feature of this extension! In any case, if you are writing Word reports I guess you shold be using Windows at some point, so why not exporting the Burp project from your Linux machine (in case you use Burp on Linux) and importing it on Burp on Windows, where you should be able to generate this report? I guess the same applies to Mac users... Not my fault, sorry, it's the third party library...</font></b>
